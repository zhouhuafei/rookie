	...
	$(function()
	{
		...
		for (var i = 0; i<$('.position').length; i++) {
			aPos.push($('.position').eq(i).offset().top-55);
		}
		//alert(aPos);
		...
		if(binfo == 6.0){
			$(window).scroll(function(){
				...
				else if($(window).scrollTop() < 715){
					$("#nav").stop().css({'position':'absolute','top':'715px'});
				}
			});
		}
		else{
			$(window).scroll(function() {
				...
				else if(w_scrollTop < 715){
					$("#nav").css({'position':'absolute','top':'715px'});
				}
			});
		}
		//----------------------------------------------
