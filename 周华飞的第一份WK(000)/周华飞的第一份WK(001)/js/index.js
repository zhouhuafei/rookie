//$(function(){
   ...
   /*
	...
	for (var i = 0; i<$('.position').length; i++) {
		aPos.push($('.position').eq(i).offset().top-113);
	}
	//alert(aPos);
	...
	if(binfo == 6.0){
		$(window).scroll(function(){
			...
			else if($(window).scrollTop() < 749){
				$("#nav").stop().css({'position':'absolute','top':'749px'});
			}
		});
	}
	else{
		$(window).scroll(function() {
			...
			else if(w_scrollTop < 749){
				$("#nav").css({'position':'absolute','top':'749px'});
			}
	  	});
	}
	*/
