...
eventBind(window,'load',function(){
	...
	/*solve*/
	(function(){
		var aBtn=document.getElementById('solve').getElementsByTagName('ol')[0].getElementsByTagName('li');
		var aCut=document.getElementById('solve').getElementsByTagName('ul')[0].getElementsByTagName('li');	
		tabSwitch(aBtn,aCut);
	})();
	/*case*/
	(function(){
			var scrollPic_02 = new ScrollPic();
			scrollPic_02.scrollContId   = "case2_ul"; //内容容器ID
			scrollPic_02.arrLeftId      = "case_prev";//左箭头ID
			scrollPic_02.arrRightId     = "case_next"; //右箭头ID
			
			scrollPic_02.frameWidth     = 1074;//显示框宽度
			scrollPic_02.pageWidth      = 542; //翻页宽度   (翻页宽度 = 显示框宽度)
...
function flowNav(oNav,aBtn,aTarget){
	...
	eventBind(window,'scroll',function(){
		...
		for(var i=0,len=aBtn.length;i<len;i++){
			if(iScrollTop>=offsetTop(aTarget[i])){
