...
Rookie.prototype.addEvent=function(obj,ev,fn){obj.attachEvent?obj.attachEvent('on'+ev,function(){fn.call(obj,window.event);}):obj.addEventListener(ev,fn,false);}
...
Rookie.prototype.tabSwitch=function(json){
	...
	if(!oParent){ throw new Error('站住,使用我插件的那枚菜鸟,你是不是忘了加参数的oParent属性,选取元素时请不要选取到了不存在的元素!'); return;}
	...
	function active(){					
		...
		setTimeout(function(){self.addClass(aCut[iCutNow],'active')},30);
	}
	oCut&&active();	
	function tab(){
		for(var i=0;i<iLen;i++){
	...
	function auto(){timer=setInterval(next,20000);}
	if(bAutoPlay){
		auto();
		oCut&&(oCut.onmouseover=function(){clearInterval(timer);})
		oCut&&(oCut.onmouseout=function(){	auto();})
	}
...
Rookie.prototype.scrollAddActive=function(obj){
	...
	for(var i=0;i<iLen;i++){iScrollTop+iClentX>=self.offset(obj[i]).y&&(self.addClass(obj[i],'active'));}
	this.addEvent(window,'scroll',function(){
...
Rookie.prototype.sliderSwitch=function(json){
	var self=this;
	var json=json||{};
	if(!json.oParent){throw new Error('oParent属性为必填属性,选取元素时请不要选取到了不存在的元素!');return;}
	var oParent=document.querySelector(json.oParent);
	var opts={
	...
	for(var i=1; i<iLen;i++){opts.aCut[i].style.left=iCutWidth+iBntWidth*(i-1)+'px';}
	for(var i=0; i<iLen;i++){
		opts.aBtn[i].index=i;
...
Rookie.prototype.delayImg=function(obj){
	var self=this;
	if(typeof obj=='string'){var obj=document.querySelectorAll(obj)}
	var iLen=obj.length;
