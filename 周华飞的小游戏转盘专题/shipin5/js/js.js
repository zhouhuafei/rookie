...
$(window).load(function(){
	var video={
			...
			if(boolean){
				for(var attr in this.data){
					for(var j=0;j<this.data[attr].img.length;j++){
						str+='<li><img alt="图片正在加载中..." _src="'+this.data[attr].img[j]+'"><p title="'+this.data[attr].txt[j]+'">'+this.data[attr].txt[j]+'</p></li>'
					}					
				}
				...
				for(var j=0;j<this.data[this.title].img.length;j++){
					...
					oLi.innerHTML='<img alt="图片正在加载中..." _src="'+this.data[this.title].img[j]+'"><p title="'+this.data[this.title].txt[j]+'">'+this.data[this.title].txt[j]+'</p>';
		...
		//图片预加载
		fnImgScroll:function(){
			var self=this;
			var iH=document.documentElement.clientHeight;
			var iScroll=document.documentElement.scrollTop||document.body.scrollTop;
			scrolls();
			this.fnAddEvent(window,'scroll',scrolls);
			function scrolls(){
				var iScroll=document.documentElement.scrollTop||document.body.scrollTop;
				for(var i=0;i<self.aLi.length;i++){
					if(iScroll+iH>=self.fnOffsetTop(self.aLi[i])&&!self.aLi[i].getElementsByTagName('img')[0].src){
						self.aLi[i].getElementsByTagName('img')[0].src=self.aLi[i].getElementsByTagName('img')[0].getAttribute('_src');	
						self.aLi[i].getElementsByTagName('img')[0].className='active';
					}	
				}	
			}
		}
