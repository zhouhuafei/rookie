...
Rookie.prototype.tabSwitch=function(json){
	...
	if(!oParent){ throw new Error('站住,使用我插件的那枚菜鸟,你是不是忘了加参数的oParent属性!'); return;}
	...
	var oCutParent=aCut?aCut[0].parentNode:null;	
	for(var i=0; i<iLen;i++){
		if(json.dir=='top'){
			aCut&&(aCut[i].style.cssText='float:none;');
		}else if((!json.dir||json.dir=='left')&&!json.bOpacity){
			aCut&&(aCut[i].style.cssText='float:left;');//for循环必须在下面那个if的上面,否则aCut的样式添加会失败,因为会受到oCutParent.innerHTML='';的影响	
			...
			oCutParent&&(oCutParent.style.position="relative");
			oCutParent&&(oCutParent.style.height=aCut[0].offsetHeight+'px');//建议在css样式里添加
	...
	if(oCutParent){
		if(!json.bOpacity){
			...
			if(oCutParent&&(oPrev||oNext||bAutoPlay)){
				oCutParent.innerHTML+=oCutParent.innerHTML;
				if(json.dir!='top')(oCutDiv.style.width=iLen*iCutWidth*2+'px');
			}
			oCutDiv.innerHTML=oCutParent.innerHTML;
			oCutParent.innerHTML='';//会影响到上面那个for循环里aCut的样式添加(如果放在aCut的样式添加之前);
			oCutParent.appendChild(oCutDiv);
		}		
	}	
	if(aBtn){
		for(var i=0;i<iLen;i++){
			...
			aBtn[i].onmouseover=function(){
				...
				iCutNow=this.index;
				tab();
				if(json.bOpacity){return;}
				oCutParent&&json.dir=='top'?oCutDiv&&self.move(oCutDiv,{top:-iCutHeight*iBtnNow}):oCutDiv&&self.move(oCutDiv,{left:-iCutWidth*iBtnNow});
			}	
		}	
	}
	function tab(){
	...
	function next(){
		...
		if(iCutNow==iLen){
			iCutNow=0;
			if(json.bOpacity){return;}			
			oCutParent&&json.dir=='top'?oCutDiv&&(oCutDiv.style.top=0):oCutDiv&&(oCutDiv.style.left=0);
		};
		if(json.bOpacity){return;}				
		oCutParent&&json.dir=='top'?oCutDiv&&self.move(oCutDiv,{top:-iCutHeight*(iCutNow+1)}):oCutDiv&&self.move(oCutDiv,{left:-iCutWidth*(iCutNow+1)});
		iCutNow++;
	}
	function prev(){
		iBtnNow--;
		...
		if(iCutNow==0){
			iCutNow=iLen;
			if(json.bOpacity){return;}			
			oCutParent&&json.dir=='top'?oCutDiv&&(oCutDiv.style.top=-iCutHeight*iCutNow+'px'):oCutDiv&&(oCutDiv.style.left=-iCutWidth*iCutNow+'px');
		};
		if(json.bOpacity){return;}			
		oCutParent&&json.dir=='top'?oCutDiv&&self.move(oCutDiv,{top:-iCutHeight*(iCutNow-1)}):oCutDiv&&self.move(oCutDiv,{left:-iCutWidth*(iCutNow-1)});
		iCutNow--;
	...
	if(bAutoPlay){
		auto();
		oCutParent&&(oCutParent.onmouseover=function(){clearInterval(timer);})
		oCutParent&&(oCutParent.onmouseout=function(){	auto();})
	}
