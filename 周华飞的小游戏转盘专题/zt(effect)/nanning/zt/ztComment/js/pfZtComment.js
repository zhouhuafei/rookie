...
$(document).ready(function(){
		...
		if(result.length!=0){
			for (var key in result)
			{
				...
				comment+='		 <li class="score"><span>评分：</span><img src="zt/ztComment/images/wujx.png"/></li>';
				comment+='		 <li class="praise"><span>[<em id="ajax_'+num+'">'+result[key]['num']+'</em>]</span><img src="zt/ztComment/images/dmz.jpg" onclick="ajax_num('+result[key]['id']+', '+num+')"/></li>';
				comment+='	   </ul>';
				comment+='	 </dt>';
		...
		blackBg.click(function(){
			...
			fm.user.value='';
			fm.info.value='';
			$("#default1").children('img').attr('src','zt/ztComment/images/star-off.png');
			$("#default2").children('img').attr('src','zt/ztComment/images/star-off.png');
			$("#default3").children('img').attr('src','zt/ztComment/images/star-off.png');
		});
...
function upPage(p){
	...
	if (nowPage < 1) {
		strS='<a onclick="upPage('+(nowPage)+')" target="_self">上一页</a>  '
	} else {	
		strS='<a onclick="upPage('+(nowPage-1)+')" target="_self">上一页</a>  '
	}
	...
	for (var i=startPage;i<=endPage;i++){
		 if (i==nowPage){
		    strC+='<a style="color:red;font-weight:700;" onclick="upPage('+i+')">'+(i+1)+'</a> ';
		 }else{
		    strC+='<a onclick="upPage('+i+')">'+(i+1)+'</a> ';
		 }
	}
	if (nowPage < PagesLen-1) {
		strE=' <a onclick="upPage('+(nowPage+1)+')">下一页</a>  ';
	} else {
		strE=' <a onclick="upPage('+(nowPage)+')">下一页</a>  ';
	}
